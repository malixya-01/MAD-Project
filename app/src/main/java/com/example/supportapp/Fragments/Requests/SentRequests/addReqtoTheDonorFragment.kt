...
class addReqtoTheDonorFragment : Fragment() {
    ...
    private fun registerEvents() {
        binding.btnSubmitReq.setOnClickListener {
            ...
            val phoneNo = binding.etReqDonorPhone.text.toString()
            val email = binding.etReqDonorEmail.text.toString()
            val message = binding.etReqDonorMsg.text.toString()
    ...
    private fun validateForm(phone: String, email: String, message: String,){
        ...
        when(contactNoValidation) {
            ...
            is ValidationResult.Invalid -> binding.etReqDonorPhone.error = contactNoValidation.errorMessage
            is ValidationResult.Valid -> contactDetailsValidity++
        }

        when(emailValidation) {
            is ValidationResult.Empty -> emptyContactDetailsCount++
            is ValidationResult.Invalid -> binding.etReqDonorEmail.error = emailValidation.errorMessage
            is ValidationResult.Valid -> contactDetailsValidity++
        }

        when(messageValidation) {
            is ValidationResult.Empty -> binding.etReqDonorMsg.error = messageValidation.errorMessage
